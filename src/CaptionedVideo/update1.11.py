...
def create_background_image_sequence(config):
    ...
    swoosh_sound_path = config.get('swoosh_sound_path', 'path/to/swoosh_sound.mp3')  # Add this line
    audio_clips = []  # Add this line
    background_clips = []
    ...
    for i, (image_path, duration) in enumerate(zip(image_paths, durations)):
        ...
        # Add swoosh sound for transitions
        if i > 0:
          swoosh_sound = AudioFileClip(swoosh_sound_path).set_start(start_time).set_duration(transition_duration)
          audio_clips.append(swoosh_sound)  # Add this line
...
def generate_final_video(config):
    ...
    # Combine background music and swoosh sounds
...
# Example usage
config = {
    'background_images': ['f1.png', 'f2.png', 'f3.png', 'f4.png', 'f5.png'],
    ...
    'swoosh_sound_path': '/content/swoosh.mp3'
}
